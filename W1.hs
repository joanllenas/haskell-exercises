...
fizzbuzz x = 
  let 
    ...
    divBy5 x = (mod) x 5 == 0
  in
    if divBy3 x && divBy5 x then "FizzBuzz" else
    if divBy3 x             then "Fizz" else
    if divBy5 x             then "Buzz" else ""
...
sumTo n = if n == 0 then n else n + sumTo (n - 1)
...
funnyCompare x y
  | x `mod` 2 == 0 && y `mod` 2 == 0 = compare x y -- both are even
  | x `mod` 2 == 0 && y `mod` 2 /= 0 = LT -- x is even
  | x `mod` 2 /= 0 && y `mod` 2 == 0 = GT -- y is even
...
tillZero :: Integer -> Bool -> String
tillZero 0 _ = "0"
tillZero n True  = (show n) ++ "," ++ (tillZero (n-1) True) -- 3,2,1,0
tillZero n False = (tillZero (n-1) False) ++ "," ++ (show n) -- 0,1,2,3

pyramid :: Integer -> String
pyramid 0 = "0"
pyramid x = (tillZero (x-1) False) ++ "," ++ (tillZero x True)
...
smallestDivisor' n i = 
  if n `mod` i == 0 then i
  else if i + 2 >= n then n
  else smallestDivisor' n (i+2)
